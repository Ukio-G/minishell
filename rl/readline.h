/* Readline.h -- the names of functions callable from within rl. */
...
/* Bindable commands for rl's interface to the command history. */
...
/* Things for vi mode. Not available unless rl is compiled -DVI_MODE. */
...
/* Undocumented; unused by rl */
extern int rl_discard_argument PARAMS((void));

/* Utility functions to bind keys to rl commands. */
...
/* The version of this incarnation of the rl library. */
extern const char *rl_library_version;		/* e.g., "4.2" */
extern int rl_readline_version;			/* e.g., 0x0402 */

/* True if this is real GNU rl. */
extern int rl_gnu_readline_p;

/* Flags word encapsulating the current rl state. */
extern unsigned long rl_readline_state;

/* Says which editing mode rl is currently using.  1 means emacs mode;
   0 means vi mode. */
...
/* The prompt rl uses.  This is set from the argument to
   rl (), and should not be assigned to directly. */
...
/* Flag to indicate that rl has finished with the current input
...
/* Display variables. */
/* If non-zero, rl will erase the entire line, including any prompt,
   if the only thing typed on an otherwise-blank line is something bound to
   rl_newline. */
extern int rl_erase_empty_line;

/* If non-zero, the application has already printed the prompt (rl_prompt)
   before calling rl, so rl should not output it the first time
   redisplay is done. */
...
/* Variables to control rl signal handling. */
/* If non-zero, rl will install its own signal handlers for
   SIGINT, SIGTERM, SIGQUIT, SIGALRM, SIGTSTP, SIGTTIN, and SIGTTOU. */
extern int rl_catch_signals;

/* If non-zero, rl will install a signal handler for SIGWINCH
...
/* If non-zero, the rl SIGWINCH handler will modify LINES and
...
/* Hook function to allow an application to set the completion word
   break characters before rl breaks up the line.  Allows
...
/* Backwards compatibility with previous versions of rl. */
...
/* Set to any quote character rl thinks it finds before any application
   completion function is called. */
extern int rl_completion_quote_character;

/* Set to a non-zero value if rl found quoting anywhere in the word to
...
/* If non-zero, rl will sort the completion matches.  On by default. */
...
/* Applications can set this to non-zero to have rl's signal handlers
   installed during the entire duration of reading a complete line, as in
   rl-6.2.  This should be used with care, because it can result in
   rl receiving signals and not handling them until it's called again
   via rl_callback_read_char, thereby stealing them from the application.
   By default, signal handlers are only active while rl is active. */
extern int rl_persistent_signal_handlers;

/* Input error; can be returned by (*rl_getc_function) if rl is reading
   a top-level command (RL_ISSTATE (RL_STATE_READCMD)). */
#define READERR			(-2)

/* Definitions available for use by rl clients. */
...
#define RL_STATE_SIGHANDLER	0x0008000	/* in rl sighandler */
